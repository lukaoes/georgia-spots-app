...
export function ResetPasswordPage() {
  ...
  const [info, setInfo] = useState<{ username: string; email: string } | null>(
    null,
  );
  ...
  if (error && !info) {
    return (
      <div className="max-w-sm mx-auto mt-16 px-4 text-center">
        ...
        <Link
          to="/login"
          className="text-[color:var(--color-clay)] underline text-sm"
        >
    ...
    return (
      <div className="max-w-sm mx-auto mt-16 px-4 text-center">
        ...
        <p className="text-sm text-[color:var(--color-ink-soft)]">
          ახლა შეგიძლიათ შეხვიდეთ ახალი პაროლით. გადამისამართდებით...
        </p>
      </div>
    );
  }
